...
int setSlices( int *iarrBegin, int *iarrEnd, int *iarrOffset ) {
  ...
  int slice, average, leftover, temp, tempOffset;
  ...
  for (slice=0 ; slice<N_SLICES; slice++) {
    ...
    iarrEnd[slice] = iarrBegin[slice] + average - 1;
...
int main(int argc, char *argv[]) {
  ...
  average = setSlices(iarrBeginSlice, iarrEndSlice, offset);
  setGrid(&crMin, &crMax, &ciMin, &ciMax, &dcr, &dci);
 
  /* memory allocation for color storage */
  storage = (double *)malloc(NX * (average + 1) * sizeof(double));

  /* master node */
  if (rank == 0) {
    master(numProcs, iarrBeginSlice, iarrEndSlice, average,
           crMin, ciMin, dcr, dci, storage, offset);
  }
  /* slave nodes  */
  else {
    slave(rank, iarrBeginSlice, iarrEndSlice, average,
